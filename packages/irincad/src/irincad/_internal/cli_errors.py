...
last, under ~50 frames of ``runpy``, the launcher, and irincad internals. A `gen_step()`
...
``--verbose`` still prints the full traceback -- when the fault IS in irincad, the internal
...
def _model_frames(exc: BaseException) -> list[traceback.FrameSummary]:
    ...
    Everything else -- the launcher, irincad, build123d/OCP, the stdlib -- is runtime: it is
    the same in every failure and says nothing about which model broke.
    """
    from irincad._internal.source_hash import is_first_party_source_file
...
def report_cli_error(
    ...
    *,
    ...
    Under ``verbose`` this is the raw traceback, unchanged -- a irincad bug needs its own
