...
* the wait notice, which is the only thing distinguishing a contended acquire from a hang --
  irincad's acquire is blocking by default, so without it a queued build emits nothing on
  either stream for as long as the peer holds the lock;
* the shape of the "a peer has it" payload, which the viewer branches on.

Not in :mod:`irincad.coordination`: that package is the cross-process protocol shared with
...
from irincad.cli_logging import CliLogger, format_elapsed
...
def deadline_ms(lock_timeout_s: float | None) -> float | None:
    """``deadline_ms`` for :func:`irincad.coordination.artifact_build`. None means block."""
...
def lock_wait_notice(logger: CliLogger | None, ref: str) -> Callable[[float], None] | None:
    ...
    Repeats on the interval :mod:`irincad.coordination.lock` applies, so a long wait keeps
