...
``__irincad__`` home, holding one content-addressed component GLB per unique
...
from irincad._internal.atomic_replace import replace_atomic, temp_suffix
...
from irincad._internal.glb import export_assembly_glb_from_scene
...
# at <folder>/__irincad__/models/<step-filename>/components/<geomHash>.glb, referenced by the
...
def _component_build_worker_count(missing_count: int) -> int:
    ...
    ``IRINCAD_COMPONENT_WORKERS`` overrides (0/1 disables). Defaults engage only
    when enough components are missing to amortize the per-worker interpreter +
    OCP import cost (~seconds each)."""
    env_value = os.environ.get("IRINCAD_COMPONENT_WORKERS", "").strip()
    if env_value:
        try:
...
def build_component_glb_from_shape(
    ...
    *,
    ...
    # otherwise scaffold a stray __irincad__/models/ tree next to the component.
...
def build_package_from_compound(
    ...
    *,
    ...
    """Emit a render package (``__irincad__/models/<entry>/``) from a baked ``Compound`` or single shape.
    ...
    ``progress`` (a :class:`irincad._internal.progress.ProgressReporter`) is driven through
    ...
    # <folder>/__irincad__/models/<key>/ and its content-addressed component GLBs live in a
