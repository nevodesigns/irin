...
the model folder's ``__irincad__/models/``, keyed by the entry filename:
...
from irincad._internal.atomic_replace import replace_atomic
from irincad._internal.file_metadata import text_to_cad_identity_metadata, write_dxf_text_to_cad_metadata
from irincad._internal.node_runtime import node_builder_script, run_node_builder
...
from irincad.drawing_checks import document_is_drawing
from irincad.render import relative_to_file, sha256_file
...
def drawing_payload_keys(descriptor: object) -> tuple[str, ...]:
    ...
    ONE definition, because irincad's producer gate and the viewer's validator must ask the
...
def build_drawing_preview(
    ...
    *,
    ...
    ``run`` is the :class:`~irincad.coordination.BuildRun` holding this package's write lock.
...
def write_drawing_package(
    ...
    *,
    ...
    gen CLI and ``irincad.dxf_artifact`` alike -- runs under ``artifact_build``. Building the
    ...
    # streamed to the mesher and nothing lands in __irincad__ but preview.glb.
    ...
    # irincad.drawing_checks.document_is_drawing), so a drawing package carries its 2D geometry
    ...
    descriptor: dict[str, object] = {
        ...
        # MODEL folder (the directory holding the .dxf.py), not the __irincad__ dir.
...
def export_drawing_dxf(script_path: Path, export_path: Path) -> Path:
    ...
    There is no cached `.dxf` to copy: __irincad__ holds only what the viewer renders, so a
    ...
    """
    from irincad._internal.generation import _normalize_dxf_payload
...
def drawing_package_current(source_path: Path) -> bool:
    ...
    # Fails closed on a missing digest, matching the viewer and irincad's own producer gate.
