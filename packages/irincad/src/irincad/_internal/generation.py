...
from irincad._internal.cli_locking import contended_payload, deadline_ms, lock_wait_notice
from irincad._internal.file_metadata import text_to_cad_identity_metadata, write_dxf_text_to_cad_metadata
from irincad._internal.package_freshness import (
    STEP_PACKAGE_VERSION,
    bake_hash_matches,
    schema_version_matches,
)
from irincad._internal.glb import build_step_topology_index_manifest
from irincad._internal.glb import read_step_topology_manifest_from_glb
from irincad._internal.glb_topology import (
    STEP_EDGE_VISIBILITY_CLASSES,
    normalize_step_edge_render_visibility_classes,
)
from irincad.coordination import (
    DRAWING_PACKAGE,
    PHASE_GENERATE,
    STEP_PACKAGE,
    ProgressEvent,
...
from irincad.cli_progress import (
    InlineProgressLine,
...
from irincad.metadata import (
    DEFAULT_MESH_ANGULAR_TOLERANCE,
    DEFAULT_MESH_TOLERANCE,
    GeneratorMetadata,
    resolve_mesh_settings,
)
from irincad.render import (
    relative_to_file,
    relative_to_cwd,
)
from irincad._internal.source_hash import (
    PythonSourceClosure,
    PythonSourceHash,
...
from irincad.step_export import build_build123d_step_scene
from irincad._internal.step_scene import (
...
from irincad._internal.generation_runner import (
...
from irincad._internal.generation_spec import (
    EntrySpec,
    GeneratedStepResult,
    _CliTargetSpec,
...
def _package_descriptor_matches_spec(
    ...
    """
    from irincad._internal.component_package import is_assembly_package, read_package_descriptor
...
def _assembly_provenance_manifest(
    ...
    *,
    ...
    from irincad._internal.glb_topology import step_topology_capabilities
    ...
    *,
    ...
    if spec.step_export_path is not None:
        def step_export_job() -> Path:
            ...
            from irincad.step_export import export_build123d_step_file
    ...
    # a component-GLB PACKAGE (a directory at __irincad__/models/<entry>: assembly.json
    ...
    def component_package_job() -> dict[str, object]:
        ...
        # import would cycle.
        from irincad._internal.component_package import build_package_from_compound
        from irincad._internal.legacy_artifacts import prune_legacy_artifacts
...
def _run_with_spec_generation_status(
    ...
    *,
    ...
    Delegates to :func:`irincad.coordination.artifact_build`, which is the SAME primitive
    ``irincad.step_artifact_cli`` uses. That shared implementation is the point: the lock, the
    ...
    ``irincad.step_artifact_cli``: 0 waits, and a positive value gives up and reports the peer
    instead. Same flag, same default, same meaning -- see :mod:`irincad._internal.cli_locking`.
    """
...
def _assembly_glb_package_current(spec: EntrySpec) -> bool:
    ...
    if spec.step_path is None:
        return False
    from irincad._internal.component_package import assembly_package_current
...
def _rebuild_child_in_subprocess(child_spec: EntrySpec) -> None:
    ...
    bootstrap = (
        "import sys\n"
        "from irincad._internal.generation import generate_step_targets\n"
...
def generate_dxf_targets(
    ...
    *,
    ...
    from irincad._internal.drawing_package import drawing_package_current
    ...
    if write_dxf:
        ...
        # default build product is the drawing package under __irincad__/models/.
        selected_specs = [
    ...
    if not force:
        ...
        if current_specs:
            from irincad._internal.drawing_package import export_drawing_dxf
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.error("irincad.generation is a library module.")
    return 2
