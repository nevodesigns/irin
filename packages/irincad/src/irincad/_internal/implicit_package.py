...
viewer shares. The package makes it an ordinary baked mesh:
    ...
    <model-folder>/__irincad__/models/<name>.implicit.js/
...
from irincad._internal.source_hash import closure_for_files, closure_hash_matches
...
def implicit_package_current(source_path: Path, *, resolution: int | None = None) -> bool:
    ...
    (``irincad._internal.package_freshness``).
    """
    resolved_source = Path(source_path).expanduser().resolve()
