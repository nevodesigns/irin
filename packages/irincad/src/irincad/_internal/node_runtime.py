...
lockfile is invisible to :func:`irincad.coordination.snapshot`, which asks the kernel -- a
...
**stdlib only**, like :mod:`irincad.coordination` beside which this sits: no OCP, build123d
...
# Env overrides, in precedence order. ``IRINCAD_NODE`` is irincad's own knob (a PyPI install
...
NODE_ENV_VARS = ("IRINCAD_NODE", "VIEWER_CAD_NODE", "CAD_NODE")
...
def cad_node_executable(repo_root: Path | str | None = None) -> str:
    ...
    symmetry with the other irincad entrypoints and is not otherwise used -- Node is a
    ...
    dependency: irincad publishes to PyPI and must not *require* Node, so nothing looks for it
    ...
    raise NodeUnavailable(
        ...
        "or point IRINCAD_NODE at an existing node binary."
    )


def node_package_root() -> Path:
    """The ``packages/`` directory this irincad was loaded from, for ``NODE_PATH``.

    Derived from THIS module's location rather than from a repo root, because it has to be
    right in both layouts: ``packages/irincad/src/irincad/_internal/node_runtime.py`` in the
    dev checkout, and ``skills/<skill>/scripts/packages/irincad/src/irincad/_internal/...`` in
    a vendored skill runtime. In both, ``parents[4]`` is the ``packages`` dir that also holds
    ``implicitjs`` and ``cadjs``. ``IRINCAD_NODE_PACKAGES`` overrides it.
    """
    override = str(os.environ.get("IRINCAD_NODE_PACKAGES") or "").strip()
    if override:
...
def node_builder_script(name: str) -> Path:
    ...
    vendors ``irincad`` but not ``cadjs`` cannot build this format at all -- so it raises with
    ...
    if not path.is_file():
        raise NodeBuilderError(
            f"Node builder is missing: {path}. The runtime that ships irincad must also ship "
            "packages/cadjs (and packages/implicitjs, which it imports); set "
            "IRINCAD_NODE_PACKAGES to the packages directory that holds them."
        )
    return path
...
def node_resolve_bootstrap() -> str:
    ...
    module, so it is present wherever irincad is -- including every skill runtime that vendors
    ``packages/irincad`` -- and needs no ``node_modules`` of its own to load.
    """
    path = Path(__file__).resolve().parent / "node_resolve_register.mjs"
    if not path.is_file():  # pragma: no cover - packaging regression
        raise NodeBuilderError(
            f"irincad's Node resolve bootstrap is missing: {path}. This file must ship with "
            "the package (see [tool.setuptools.package-data] in irincad's pyproject.toml)."
        )
...
def run_node_builder(
    ...
    *,
    ...
    ``run`` is the :class:`~irincad.coordination.BuildRun` yielded by ``artifact_build`` --
    ...
    **Protocol** -- one JSON object per stdout line:
    ...
    ``irincad_bridge`` already applies to cold-CLI stdout, so an incidental ``console.log`` from
