...
into itself, exactly as it already imports ``irincad._internal.source_hash``. Sharing the
...
package is stale and gets rebuilt (``__irincad__/`` is a gitignored per-machine cache, so
...
def schema_version_matches(descriptor: Mapping[str, Any], expected: int) -> bool:
    ...
    ``irincad.coordination.record``'s schema gate: a missing, non-integer, or older
