...
class PythonSourceHash:
    ...
    paths come from :func:`irincad.render.relative_to_file`, anchored on the model folder --
    see ``tests/python/packages/irincad/test_package_portability.py``.
    """
...
def _runtime_roots() -> tuple[Path, ...]:
    """Directories holding the RUNNING generation runtime itself: the active irincad
    ...
    never a model's freshness input. In production irincad lives in site-packages and
...
def evict_first_party_modules() -> tuple[str, ...]:
    ...
    and third-party modules (irincad, build123d, OCP, ...) are never touched: they
