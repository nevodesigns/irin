...
TEXT_TO_CAD_GENERATOR = "irincad"
TEXT_TO_CAD_GENERATOR_PROPERTY = "irincad:generator"
TEXT_TO_CAD_ENTRY_KIND_PROPERTY = "irincad:entryKind"
TEXT_TO_CAD_SOURCE_PATH_PROPERTY = "irincad:sourcePath"
TEXT_TO_CAD_SOURCE_HASH_PROPERTY = "irincad:sourceHash"
TEXT_TO_CAD_METADATA_GROUP = "irincad metadata"
...
def inject_text_to_cad_step_metadata(
    ...
    *,
    ...
    if normalized_entry_kind is None:
        raise ValueError(f"Unsupported irincad STEP entry kind: {entry_kind!r}")
    normalized_source_path = normalize_text_to_cad_source_path(source_path)
    if source_path and normalized_source_path is None:
        raise ValueError(f"Unsupported irincad STEP source path: {source_path!r}")
...
def read_text_to_cad_step_metadata_text(step_text: str) -> dict[str, str]:
    ...
    for match in re.finditer(
        ...
        elif property_name in {TEXT_TO_CAD_ENTRY_KIND_PROPERTY, "irincad:entry_kind"}:
