...
from irincad.catalog import IRINCAD_DIRNAME
from irincad.catalog import IRINCAD_MODELS_DIRNAME

from irincad._internal.step_scene_loader import _location_from_transform_matrix, _relative_path_from_directory, _shape_hash, _step_hash, load_step_scene
from irincad._internal.step_scene_types import ColorRGBA, LoadedStepScene, OccurrenceNode, STEP_SCENE_CACHE_SCHEMA_VERSION, _STEP_SCENE_CACHE_BINTOOLS_VERSION, _identity_transform_matrix


_STEP_SCENE_CACHE_DIRNAME = IRINCAD_DIRNAME
_STEP_SCENE_CACHE_MODELS_DIRNAME = IRINCAD_MODELS_DIRNAME
...
def _step_scene_cache_dir(step_path: Path, step_hash: str) -> Path | None:
    ...
    Written under
    ``<base>/__irincad__/models/<step-filename>/scene/v<schema>-<hash>`` so it sits
    inside the same per-model ``__irincad__/models`` home as the component-GLB render
    ...
    if configured:
        ...
        base = Path(tempfile.gettempdir()).resolve() / "irincad-step-scene-cache"
    else:
...
def _prune_step_scene_cache_siblings(cache_dir: Path) -> None:
    ...
    keeping only the just-written ``cache_dir`` so __irincad__ does not accumulate."""
    try:
