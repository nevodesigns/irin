...
from irincad.metadata import MeshSettings

from irincad._internal.step_scene_cache import load_step_scene_cached
from irincad._internal.step_scene_geometry import _bbox_from_points, _bbox_from_shape, _merge_bbox, _transform_bbox
from irincad._internal.step_scene_loader import _located_shape, _selector_id
from irincad._internal.step_scene_types import AdaptiveMeshResolution, LoadedStepScene, OccurrenceNode, _enum_name
...
def import_step(step_path: Path, *, label: str | None = None) -> Any:
    """``build123d.import_step`` backed by the inline ``__irincad__`` scene cache.
