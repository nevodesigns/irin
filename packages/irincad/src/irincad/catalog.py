...
# The per-folder generated-artifact home for irincad outputs (render packages, scene
# caches, generation locks), living beside the CAD sources it derives from. Gitignored.
IRINCAD_DIRNAME = "__irincad__"
IRINCAD_MODELS_DIRNAME = "models"
...
class StepImportOptions:
    ...
    # configured here — they are one-off exports owned by irincad.step_export_target
...
def render_package_dir(entry_path: Path) -> Path:
    ...
    # INSIDE the per-folder __irincad__ directory, keyed by the ENTRY filename (the on-disk
    ...
    return (base.parent / IRINCAD_DIRNAME / IRINCAD_MODELS_DIRNAME / base.name).resolve()
...
def _iter_python_sources(root: Path) -> tuple[CadSource, ...]:
    ...
    for script_path in _iter_paths(root, "*.py"):
        ...
        try:
            ...
            print(f"[irincad] skipping invalid CAD source: {exc}", file=sys.stderr)
            continue
        if source is not None:
            sources.append(source)
    return tuple(sources)
