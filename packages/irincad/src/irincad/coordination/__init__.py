...
ONE implementation, imported by both sides. The producer (irincad, from a CLI or the CAD
...
never drag OCP/build123d/ezdxf into that process. ``irincad/__init__.py`` defers all of its
own imports, so ``import irincad.coordination`` costs milliseconds and pulls in no CAD
runtime. That invariant is pinned by
``tests/python/packages/irincad/test_coordination_is_stdlib_only.py``; do not add a
...
from irincad.coordination import record as _record
from irincad.coordination.kinds import (
    DRAWING_PACKAGE,
    IMPLICIT_PACKAGE,
    PHASE_BROWSER,
    PHASE_RENDER,
...
from irincad.coordination.lock import (
    Contended,
    exclusive,
    locking_available,
    new_run_id,
...
from irincad.coordination.paths import (
    generator_lock_path,
    generator_status_path,
    status_path,
    write_lock_path,
)
from irincad.coordination.phases import (
    NULL_PROGRESS,
    PHASE_COMPONENTS,
    PHASE_DONE,
    PHASE_FINALIZE,
...
class BuildRun:
    ...
    def __init__(self, reporter: Any, run_id: str | None, *, degraded: bool = False) -> None:
        ...
        # filesystem that refuses advisory locks, an unwritable __irincad__). The build goes
...
_CURRENT_BUILD: contextvars.ContextVar[BuildRun | None] = contextvars.ContextVar(
    "irincad_current_build", default=None
)
...
def artifact_build(
    ...
    *,
    ...
    :func:`irincad.coordination.lock.exclusive`.
    """
...
def require_write_lock(output_dir: Path | str) -> bool:
    ...
    Raises under ``IRINCAD_STRICT_LOCKS=1`` (tests and CI); otherwise warns and continues,
    because a missing lock must never be the reason a user's build fails.
    """
    from irincad.coordination.lock import locking_available
    ...
    from irincad.coordination.lock import _HELD  # noqa: PLC2701 - same package
    ...
    message = (
        ...
        "Wrap the write in irincad.coordination.artifact_build()."
    )
    if os.environ.get("IRINCAD_STRICT_LOCKS") == "1":
