...
One derivation, imported by BOTH the producer (irincad) and the reader (the CAD Viewer's
server), so the two can no longer disagree about which file to look at. Before this module
the derivation existed twice -- ``irincad._internal.generation_status.generation_lock_path``
...
the gitignored ``__irincad__`` tree next to the package they describe. For an output dir
``<folder>/__irincad__/models/<name>.step`` the files are::

    <folder>/__irincad__/models/.<name>.step.generation.lock           writer sentinel
    <folder>/__irincad__/models/.<name>.step.generator.lock            generator sentinel
    <folder>/__irincad__/models/.<name>.step.generation.progress.json  status record
...
# The writer sentinel's name is FROZEN. A bundled skill carrying an older irincad and a
...
#
# One rollout gap is known and accepted, on Windows only: irincad <= 0.4.11 locked this file
...
# there. Narrow enough to accept -- it needs two different irincad installs writing one
