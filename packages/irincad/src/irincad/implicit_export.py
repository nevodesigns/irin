...
The implicit analogue of :mod:`irincad.step_export_target`, and for the same reason: the CAD
...
else entirely and rewrites nothing inside ``__irincad__``, so claiming the write lock would
...
from irincad._internal.implicit_package import is_implicit_source_path
from irincad._internal.node_runtime import (
    NodeBuilderError,
    cad_node_executable,
    node_child_env,
    node_package_root,
...
from irincad.coordination import IMPLICIT_PACKAGE, generator_busy
...
def implicit_export_cli() -> Path:
    ...
    vendored skill runtime. Missing is a PACKAGING failure -- a runtime that ships irincad
    ...
    if not path.is_file():
        raise NodeBuilderError(
            f"The implicit CAD export CLI is missing: {path}. The runtime that ships irincad "
            "must also ship packages/implicitjs; set IRINCAD_NODE_PACKAGES to the packages "
...
def _last_json_object(stdout: str) -> dict[str, object]:
    """The last JSON object printed on stdout, matching the contract every other irincad
    export CLI uses. Tolerant of incidental output before it, exactly as
    ``irincad_bridge`` and ``run_node_builder`` already are."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m irincad.implicit_export",
...
def run_cli_payload(
    ...
    *,
    ...
    ignored for the same reason :mod:`irincad.implicit_artifact` ignores it: the model is a JS
