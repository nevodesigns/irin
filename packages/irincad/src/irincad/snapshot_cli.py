...
It lives in irincad rather than in the CAD skill because a skill may not import another
skill's code (AGENTS.md), and the robot resolver alone is needed by three skills at once.
The split against :mod:`irincad.snapshot_core` is by ROLE, not by format: the core owns the
...
import irincad.cad_ref_syntax as cad_ref_syntax
import irincad.lookup as lookup
from irincad.catalog import render_package_dir
from irincad.step_targets import ResolvedStepTarget, StepTopologyArtifact, StepTopologyArtifactError

from irincad.cli_logging import CliLogger
from irincad.coordination import PHASE_BROWSER, SNAPSHOT, ProgressReporter
from irincad.cli_progress import cli_progress_line
from irincad.snapshot_core import (
    THEME_OPTION_KEYS,
...
def load_ensure_step_topology_artifact():
    ...
    if ensure_step_topology_artifact is None:
        from irincad.step_artifacts import ensure_step_topology_artifact as imported_ensure
...
def artifact_selector_index(artifact: StepTopologyArtifact | None) -> lookup.SelectorIndex | None:
    ...
    # every ref `--mode list` had just handed out. See `irincad.assembly_lookup`.
...
def normalize_selection_selector(
    ...
    *,
    ...
    if parsed.label:
        ...
        from irincad.label_refs import LabelResolutionError, resolve_label_selectors
