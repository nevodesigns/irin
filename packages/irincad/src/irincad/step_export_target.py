...
It is deliberately distinct from :mod:`irincad.step_artifact_cli`, which only (re)builds the
per-folder ``__irincad__`` viewer GLB/topology package beside the source. This module
...
from irincad.catalog import source_from_path
...
from irincad._internal.glb import export_native_glb_from_scene
from irincad.metadata import normalize_mesh_numeric
from irincad.step_artifact_cli import _build_entry_spec, _cad_ref_for_step, infer_entry_kind
...
from irincad._internal.stl import export_part_stl_from_scene
from irincad._internal.threemf import export_part_3mf_from_scene
...
def _resolve_spec_and_scene(
    ...
    *,
    ...
    and classify it via :func:`irincad.step_artifact_cli.infer_entry_kind`.
    """
    if source_path is not None:
        ...
        # generator does not itself resolve to (mirrors irincad.step_artifact_cli).
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m irincad.step_export_target",
...
def export_model_to_path(
    *,
    ...
    :func:`irincad.generation.run_script_generator`."""
    if logger is None:
...
def export_cad_target(
    ...
    *,
    ...
    Writes no ``.step``, no ``__irincad__`` render package, and no other beside-source
    artifacts."""
    if logger is None:
