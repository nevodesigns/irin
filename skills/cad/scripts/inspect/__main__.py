...
# Warm-daemon shim: must run BEFORE the cli import below (which loads irincad at
# module import time). The daemon sets IRINCAD_DAEMON_CHILD so it never
# recurses; the stdlib-only client keeps the cold path overhead-free.
if os.environ.get("IRINCAD_WARM") == "1" and not os.environ.get("IRINCAD_DAEMON_CHILD"):
    scripts_dir = str(Path(__file__).resolve().parents[1])
    if scripts_dir not in sys.path:
        sys.path.insert(0, scripts_dir)
    from irincad_daemon.client import run_via_daemon

    warm_exit = run_via_daemon("inspect", sys.argv[1:], os.getcwd())
    if warm_exit is not None:
        raise SystemExit(warm_exit)

# Prefer the skill's bundled irincad over any pip-installed copy, exactly like
# snapshot's entry: the vendored scripts/packages/irincad is the version this
