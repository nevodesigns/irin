...
The tool launchers' ``IRINCAD_WARM`` shim imports this module BEFORE any heavy
...
# Trees whose .py mtimes define the daemon's version token: the shared irincad
...
_VERSION_TREES = ("packages/irincad/src/irincad", "skills/cad/scripts")

_RESTART = object()


def socket_path() -> Path:
    override = os.environ.get("IRINCAD_DAEMON_SOCKET")
    if override:
        return Path(override)
    digest = hashlib.sha256(str(REPO_ROOT).encode("utf-8")).hexdigest()[:12]
    return Path(os.environ.get("TMPDIR") or "/tmp") / f"irincad-daemon-{digest}.sock"
...
def request_timeout() -> float:
    ...
    ``IRINCAD_DAEMON_TIMEOUT`` overrides; 0 or a negative value disables the
    deadline entirely (the old, unbounded behaviour) for anyone who genuinely
    wants to wait out a very long queued build.
    """
    raw = os.environ.get("IRINCAD_DAEMON_TIMEOUT", "").strip()
    if not raw:
...
def compute_version_token(root: Path | None = None) -> int:
    ...
    for tree in _VERSION_TREES:
        # os.walk does not follow directory symlinks, so the scripts/packages/irincad
        # symlink does not double-count the irincad tree.
...
def run_via_daemon(tool: str, argv: list[str], cwd: str | None = None) -> int | None:
    ...
    if os.environ.get("IRINCAD_WARM") != "1" or os.environ.get("IRINCAD_DAEMON_CHILD"):
        return None
...
def _spawn_daemon(sock_path: Path) -> subprocess.Popen | None:
    env = dict(os.environ)
    env["IRINCAD_DAEMON_CHILD"] = "1"
    env.setdefault("IRINCAD_DAEMON_SOCKET", str(sock_path))
    try:
...
def _run_request(conn: socket.socket, payload: dict) -> int | object | None:
    ...
    try:
        ...
        print(
            f"irincad-daemon: no response for {timeout:.0f}s; running cold "
            "(set IRINCAD_DAEMON_TIMEOUT to change or 0 to wait indefinitely)",
            file=sys.stderr,
            flush=True,
        )
        return None
