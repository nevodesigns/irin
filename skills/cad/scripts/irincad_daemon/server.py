...
One long-lived process imports irincad / OCP / build123d ONCE and then services
...
sessions (``IRINCAD_WARM=1``) skip the multi-second interpreter+OCP startup on
every call. The daemon runs with ``IRINCAD_DAEMON_CHILD=1`` so the launcher shim
...
determinism rides on irincad's pre-run first-party module eviction (see
``irincad._internal.generation.run_script_generator``); the daemon additionally
...
CADPY_SRC_DIR = PACKAGES_DIR / "irincad" / "src"
...
# the daemon skips the launchers' IRINCAD_WARM shim and their name-colliding
...
def _log(message: str) -> None:
    print(f"[irincad-daemon] {message}", file=sys.__stderr__, flush=True)


def _idle_timeout() -> float:
    try:
        return max(1.0, float(os.environ.get("IRINCAD_DAEMON_IDLE_TIMEOUT", "")))
    except ValueError:
...
def _warm_imports() -> None:
    ...
    importlib.import_module("irincad.generation")
...
def _handle_request(
    ...
    try:
        if tool not in _TOOL_IMPORTS or not isinstance(argv, list):
            err.write(f"irincad-daemon: invalid request for tool {tool!r}\n")
        else:
...
def serve() -> int:
    os.environ["IRINCAD_DAEMON_CHILD"] = "1"
...
USAGE = """\
irincad-daemon takes no arguments.

It is the warm-process server, started for you by irincad_daemon.client when
IRINCAD_WARM=1 -- not a command to run by hand. It sits in scripts/ beside the
